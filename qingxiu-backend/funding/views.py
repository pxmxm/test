...
class GrantSubjectViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=False, methods=['get'], url_path='obtain_unit')
    def obtain_unit(self, request, *args, **kwargs):
        grant_subject_id = request.query_params.dict().get('grantSubjectId')
        unit_name = [i.unitName for i in AllocatedSingle.objects.filter(grantSubject=grant_subject_id)]
        return Response({"code": 0, "message": "ok", "detail": unit_name}, status=status.HTTP_201_CREATED)
    ...
    def detection(self, request, *args, **kwargs):
        ...
        if AllocatedSingle.objects.filter(grantSubject=grant_subject.id, storage=False):
            ...
            return Response({"code": 1, "message": "ok"}, status=status.HTTP_201_CREATED)

    @action(detail=False, methods=['post'], url_path='determine_upload')
    def determine_upload(self, request, *args, **kwargs):
        grant_type = request.data['grantType']
        grant_subject = GrantSubject.objects.get(subject=request.data['subjectId'], grantType=grant_type)
        if AllocatedSingle.objects.filter(grantSubject=grant_subject.id, storage=False):
            return Response({"code": 1, "message": "请核对申请拨款单是否全部填写完毕"}, status=status.HTTP_201_CREATED)
        ...
        grant_subject.attachments = request.data['attachments']
    ...
    def change_user_funding_no_show(self, request):
        ...
        subject = Subject.objects.order_by('-project__category__batch__annualPlan',
                                           '-contract_subject__contractNo').filter(project__charge=charge)
        for s in subject:
                        ...
                        for ut in AllocatedSingle.objects.filter(grantSubject=i.id):
                            for funding in contract_content.unitFunding:
                                if funding['unit'] == ut.unitName:
                                    if i.subject.subjectState in ['验收不通过', '项目终止', '逾期未结题']:
                                        money = Decimal(funding['first'])
...
class AllocatedSingleViewSet(mongodb_viewsets.ModelViewSet):
    ...
    def attachment_show(self, request):
        subject_id = request.query_params.dict().get('subjectId')
        try:
            grant_subject = GrantSubject.objects.filter(subject_id=subject_id, state='通过').values('attachments')
